...
build_legacy() {
    ...
    # Strip CRT function objects from libwine_port.a so the linker resolves
    # them from libmsvcrt.a (import thunks → msvcrt.dll). Wine's portable C
    # library provides memcpy/memmove/strcasecmp etc. as local implementations
    # which shadow the msvcrt.dll imports the reference DLLs use.
    if [ -f libs/port/libwine_port.a ]; then
        local port_tmp="${TMPDIR:-/tmp}/port_$$.a"
        objcopy --strip-symbol _memcpy --strip-symbol _memmove \
                --strip-symbol _memcmp --strip-symbol _memset \
                --strip-symbol _strlen --strip-symbol _strcmp \
                --strip-symbol _strcpy --strip-symbol _strcat \
                --strip-symbol _strchr --strip-symbol _strrchr \
                --strip-symbol _strstr --strip-symbol _strncmp \
                --strip-symbol _strcspn --strip-symbol _strnicmp \
                --strip-symbol _strcasecmp --strip-symbol _strncasecmp \
                libs/port/libwine_port.a "$port_tmp" 2>/dev/null \
            && mv "$port_tmp" libs/port/libwine_port.a \
            && echo "    Stripped CRT from libwine_port.a"
    fi
